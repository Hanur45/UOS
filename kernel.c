...
//             UOS DEVS ONLY
//
//       RELEASED UNDER UOS PROPERTY
//
//             KERNEL RUNTIME
//
//         UOS ALL RIGHTS RESERVED
//
#include "kernel.h"
#include "utils.h"
...
void kernel_entry()
{
  const char*str = "UOS test";

  init_vga(WHITE, BLACK);
  gotoxy((VGA_MAX_WIDTH/2)-strlen(str), 1);
  print_color_string("UOS TEST", WHITE, BLACK);
  
  draw_box(BOX_DOUBLELINE, 0, 0, BOX_MAX_WIDTH, BOX_MAX_HEIGHT, BRIGHT_GREEN, BLACK);
  //UOS make window
  draw_box(BOX_SINGLELINE, 5, 3, 20, 5, YELLOW, BLACK);
  draw_box(BOX_SINGLELINE, 5, 1, 20, 1, YELLOW, BLACK);
  //set xy to print on
  gotoxy(6, 2);
  print_color_string("INIT", BRIGHT_RED, BLACK);
  gotoxy(10, 6);
  print_color_string("kernel INIT", BLUE, BLACK);
  init_gdt();
  gotoxy(10, 7);
  print_color_string("GDT/IDT INIT", BRIGHT_GREEN, BLACK);
  init_idt();

  draw_box(BOX_SINGLELINE, 8, 2, 20, 8, YELLOW, BLACK);
